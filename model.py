...
class CNNRunner(Runner):
    def run_criterion(self, batch):
        ...
        # log some info
        self.events[self._phase_name]['loss'].append(
    ...
    def output_log(self, **kwargs):
        ...
        if visualize:
            ...
            ax2.hist(class_0_scores, bins=50, range=[
                     0, 1.01], color='r', alpha=0.7, label='cats')
            ax2.hist(class_1_scores, bins=50, range=[
                     0, 1.01], color='g', alpha=0.7, label='dogs')
            ax2.legend()
...
class ModelWrapper:
    ...
    def train(self):
        ...
        runner = CNNRunner(model_resnet18, opt_resnet,
                           self.device, hp.model_file_name)
        runner.train(train_batch_gen, test_batch_gen,
                     n_epochs=hp.epoch_num, visualize=False)
    ...
    def evaluate_model(self):
        ...
        val_stats = runner.validate(
            val_batch_gen, model, 'val', dump_csv='validate_res.csv')
