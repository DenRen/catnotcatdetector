...
- [x] **II**-ой этап: `dvc`, `hydra`, `logging` и `onnx`
  ...
  - [x] Добавить логирование обучения
  - [x] Перенести гиперпараметры в yaml конфиги hydra
  - [x] Добавить экспорт модели в `onnx` и запускать инференс через неё
- [ ] **III**-ий этап: `Triton`
  - [ ] [`TODO`][link_todo]
- [ ] **IV**-ый этап: `OS`
...
- [x] Реализовать обучение через `pytorch lightning`
- [x] Сделать систему буквально запускаемой от одной кнопки
- [x] Перенести данные на `Google Drive` или `Yandex Drive`
- [x] Исправить проблему того, что `pre-commit` не проверяет директорию `src`
